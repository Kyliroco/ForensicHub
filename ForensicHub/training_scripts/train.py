...
def main(
    ...
    if isinstance(train_dataset_args, list):
        ...
        for t_args in train_dataset_args:
            ...
            if dataset_percentage is not None and dataset_percentage < 100:
                ...
                random.Random(getattr(args, 'seed', 42)).shuffle(indices)
    ...
    for test_args in test_dataset_args:
        ds_name = test_args["dataset_name"]
        if "evaluator" in test_args:
            ds_evaluators = []
            for eva_args in test_args["evaluator"]:
                ds_evaluators.append(build_from_registry(EVALUATORS, eva_args))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.test_period == 0 or epoch + 1 == args.epochs:
            ...
            for test_dataset_name, test_dataloader in test_dataloaders.items():
                ...
                for evaluator in ds_evaluator_list:
            ...
            metrics_dict = {
                metric: {dataset: values[dataset][metric] for dataset in values if metric in values[dataset]}
                for metric in {m for d in values.values() for m in d}
            }
            # Calculate the mean of each metric across datasets that have it
            metric_means = {
            ...
            # Collect all unique evaluator names across all datasets
            all_evaluator_names = list({
                e.name for evals in per_dataset_evaluators.values() for e in evals
            })
            ...
            if evaluate_metric_value > best_evaluate_metric_value:
                ...
                print(
                    f"Best {' '.join(all_evaluator_names)} = {best_evaluate_metric_value}"
                )
                ...
                print(
                    f"Average {' '.join(all_evaluator_names)} = {evaluate_metric_value}"
                )
